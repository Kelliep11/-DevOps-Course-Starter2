...
def test_view_model_todo_items_property_only_shows_todo_status_items():
    ...
    assert returned_items[0].status == "To Do"

def test_view_model_todo_items_property_only_shows_done_status_items():
    #Given
    test_items = [
        Item("123", "To Do Item", "To Do"),
        Item("345", "Complete Item", "Done")
    ...
    # When
    returned_items = test_view_model.done_items
    # Then
    assert len(returned_items) == 1
    assert returned_items[0].status == "Done"